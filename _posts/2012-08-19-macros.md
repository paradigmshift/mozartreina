...
Besides being shorter and more concise, we didn't have to rely on any custom-defined macro. This makes it easier to maintain, expecially if there are other members in the team or if the code will be passed on to another programmer in the future.
...
For example, let's write a macro called `if-var`. `if-var` returns the value of the argument passed to it or a `nil` if it is a nil (think empty list, kind of similary to `(nullp)`):
...
- [On Lisp](http://www.bookshelf.jp/texi/onlisp/onlisp_8.html#SEC49)